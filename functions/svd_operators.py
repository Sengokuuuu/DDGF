...
class SRConv(A_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, stride=1):
        ...
        # get the svd of the 1D conv
        self.U_small, self.singulars_small, self.V_small = torch.svd(A_small, some=False)
        self.singulars_small_orig = self.singulars_small.clone()  # 保存原始奇异值
        ZERO = 3e-2
        self.singulars_small[self.singulars_small < ZERO] = 0
        # calculate the singular values of the big matrix
        ...
        self._singulars_orig = torch.matmul(self.singulars_small_orig.reshape(small_dim, 1),
                                            self.singulars_small_orig.reshape(1, small_dim)).reshape(small_dim ** 2)
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        temp_vec = self.Vt(vec)

        singulars = self._singulars_orig.repeat(self.channels)
        num_singular_values = singulars.shape[0]

        # 1. 计算低维的缩放系数 lambda_t_small
        lambda_t_small = torch.ones_like(singulars)

        inverse_singulars = 1. / singulars
        inverse_singulars[singulars == 0] = 0.

        if a != 0 and sigma_y != 0:
            change_index = (sigma_t < a * sigma_y * inverse_singulars)
            lambda_t_small[change_index] = (singulars[change_index] * sigma_t * (1 - eta ** 2) ** 0.5 / (a * sigma_y))

        # 2. 创建一个与 temp_vec 同尺寸、元素全为 1 的完整缩放张量
        lambda_t_full = torch.ones_like(temp_vec)

        # 3. 将低维缩放系数填充到完整张量的前部
        lambda_t_full[:, :num_singular_values] = lambda_t_small.reshape(1, -1)

        # 4. 执行尺寸匹配的元素乘法
        temp_vec = temp_vec * lambda_t_full

        return self.V(temp_vec)

    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        temp_vec = self.Vt(vec)
        temp_eps = self.Vt(epsilon)

        singulars = self._singulars_orig.repeat(self.channels)
        num_singular_values = singulars.shape[0]

        # 1. 计算低维的缩放系数 d1_t_small 和 d2_t_small
        d1_t_small = torch.ones_like(singulars)
        d2_t_small = torch.ones_like(singulars)
        ...
        # 根据您之前的逻辑进行计算
        if a != 0 and sigma_y != 0:
            cond_less = (sigma_t < a * sigma_y * inverse_singulars) & (singulars != 0)
            cond_greater = (sigma_t >= a * sigma_y * inverse_singulars) & (singulars != 0)

            d1_t_small[cond_less] = sigma_t * eta
            d1_t_small[cond_greater] = torch.sqrt(sigma_t ** 2 - (a * sigma_y * inverse_singulars[cond_greater]) ** 2)

            d2_t_small[cond_less] = 0
            d2_t_small[cond_greater] = 0

        # 对于 s=0 的情况 (零空间)
        cond_zero = (singulars == 0)
        d1_t_small[cond_zero] = sigma_t * eta
        d2_t_small[cond_zero] = sigma_t * (1 - eta ** 2) ** 0.5

        # 2. 创建与 temp_vec/temp_eps 同尺寸的完整缩放张量
        #    并用零空间的默认值进行初始化
        d1_t_full = torch.full_like(temp_vec, sigma_t * eta)
        d2_t_full = torch.full_like(temp_eps, sigma_t * (1 - eta ** 2) ** 0.5)

        # 3. 将低维缩放系数填充到完整张量的前部
        d1_t_full[:, :num_singular_values] = d1_t_small.reshape(1, -1)
        d2_t_full[:, :num_singular_values] = d2_t_small.reshape(1, -1)

        # 4. 执行尺寸匹配的元素乘法
        out_vec = self.V(temp_vec * d1_t_full)
        out_eps = self.V(temp_eps * d2_t_full)
